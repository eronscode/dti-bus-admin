    ...
    addWithOthers = (data) => {
      ...
      PaymentHandler = (type) => {
            ...
            if(type === "online"){
                ...
                const newData = {
                    ...
                    onSuccess: (response) => {
                        formData = {
                            numberofseats:1,
                            paymentreferenceid:response.reference,
                            usergoing:true,
                            corider: []
                        }
        ...
        if(this.state.bookingState === 3 && this.state.withOthersData.length !== 0){
            ...
            price = this.props.tripDetails.price * this.state.withOthersData.length;
    ...
    render(){
        ...
        if(this.props.tripDetails && this.props.tripDetails.length !== 0){
            ...
            <div className="col-md-4">
                ...
                paymentProps={this.state.paystackProps}  />
            </div>
            </div>
            )
        }
        ...
        if(this.state.initiatePayment === true){
            
            modal = <Modal show={this.state.initiatePayment} modalClosed={() => {
                this.setState({initiatePayment: false})
            }}>
            <div style={{textAlign:'center'}}>
                
                <h5 style={{margin:'23px 0'}}>You're almost done! Click the button below to continue</h5>
                <PaystackConsumer {...this.state.paystackProps} >
                    {({ initializePayment }) => <button className={classes.proceedBtn} onClick={() => {
                        initializePayment() 
                        this.setState({initiatePayment: false})
                         }}>Proceed with payment <i className="fa fa-arrow-circle-o-right" aria-hidden="true"></i></button> }
                </PaystackConsumer> 
            </div>
        </Modal>
        }
